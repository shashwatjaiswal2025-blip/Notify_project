...
- 3 (normal): General information or FYI without immediate action.
...
def call_ollama(prompt: str) -> dict:
    # JSON mode + non-stream so we get a single JSON string back
    payload = {
        "model": OLLAMA_MODEL,
        "prompt": prompt,
        "format": "json",
...
# NEW: sample endpoint to trigger a demo from the browser/curl
...
# NEW: startup check to confirm Ollama is reachable and model is ready
@app.on_event("startup")
def on_startup():
    try:
        # Light ping: list tags to confirm server is up
